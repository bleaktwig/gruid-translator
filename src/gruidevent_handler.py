...
def _gen_bts(hits, deltax, deltay, deltaz, dt, dx, dy, dz):
...
def _gen_pts(hits, deltax, deltay, dt, dx, dy):
    """
...
def generate_event(hits, in_nrows, in_ncols, dt, dx, dy, dz):
    ...
    event[c.S_GRUIDH1] = _gen_pts(hits[c.S_PHOTONH1], c.DELTAX(in_ncols), c.DELTAY(in_nrows),
                                  dt, dx, dy)
    event[c.S_GRUIDH2] = _gen_pts(hits[c.S_PHOTONH2], c.DELTAX(in_ncols), c.DELTAY(in_nrows),
                                  dt, dx, dy)
    event[c.S_GRUIDHB] = _gen_bts(hits[c.S_MASSHITS], c.DELTAX(in_ncols), c.DELTAY(in_nrows),
                                  c.DELTAZ, dt, dx, dy, dz)
    return event
